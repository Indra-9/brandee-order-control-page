...
export default function WebhookManager() {
  ...
  const fetchWebhooks = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('webhook_endpoints').select('*').order('created_at', {
        ascending: false
      });
  ...
  if (isLoading) {
    return <Card className="bg-brandae-gray border-brandae-green/20">
  ...
  return <Card className="bg-brandae-gray border-brandae-green/20">
      <CardHeader className="rounded">
        <div className="flex items-center justify-between">
          ...
          <Button onClick={() => setIsAdding(!isAdding)} className="bg-brandae-green text-brandae-dark hover:bg-brandae-green/90 rounded">
      ...
      <CardContent className="space-y-6">
        ...
        height: 0
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              <div>
                <Input placeholder="Webhook Name (e.g., Slack Notifications)" {...register('name', {
              required: 'Name is required'
            })} className="bg-brandae-dark border-white/10 text-white" />
                {errors.name && <p className="text-red-500 text-sm mt-1">{errors.name.message}</p>}
              </div>
              <div>
                <Input placeholder="https://hooks.slack.com/services/..." {...register('url', {
        ...
        {webhooks.length === 0 ? <div className="text-center py-8 text-gray-400">
            ...
            <Table>
              ...
              <TableBody>
                {webhooks.map((webhook, index) => <motion.tr key={webhook.id} initial={{
              opacity: 0,
              y: 20
            }} animate={{
              opacity: 1,
              ...
              delay: index * 0.1
                    ...
                    <TableCell>
                      <Button size="sm" variant="outline" onClick={() => deleteWebhook(webhook.id)} className="border-red-500/50 text-red-400 hover:bg-red-500/10 rounded">
