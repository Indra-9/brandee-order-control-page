...
const IntegrationManager = () => {
  ...
  const toggleFeatured = async (integration: Integration) => {
    try {
      const { error } = await supabase
        .from('integrations')
        .update({ is_featured: !integration.is_featured })
        .eq('id', integration.id);

      if (error) throw error;

      setIntegrations(integrations.map(i => 
        i.id === integration.id ? { ...i, is_featured: !i.is_featured } : i
      ));
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex flex-wrap gap-2 mb-2">
                          ...
                          <Badge 
                            ...
                            className={integration.is_active ? "bg-green-600" : "bg-gray-600"}
                          >
                  ...
                  <CardContent>
                    <div className="flex gap-2 flex-wrap">
                      ...
                      <Button 
                        size="sm" 
                        variant="outline" 
                        onClick={() => toggleFeatured(integration)}
                        className="border-brandae-purple/50 text-brandae-purple hover:bg-brandae-purple/10"
                      >
