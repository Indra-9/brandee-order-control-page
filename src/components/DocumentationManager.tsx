...
const DocumentationManager = () => {
  ...
  const filterDocumentation = () => {
    let filtered = docs;
    if (searchTerm) {
      filtered = filtered.filter(doc => doc.title.toLowerCase().includes(searchTerm.toLowerCase()) || doc.excerpt?.toLowerCase().includes(searchTerm.toLowerCase()) || doc.tags.some(tag => tag.toLowerCase().includes(searchTerm.toLowerCase())));
    }
    if (filterCategory) {
  ...
  const handleDeleteDoc = async (docId: string) => {
    ...
    try {
      const {
        error
      } = await supabase.from('documentation').delete().eq('id', docId);
      if (error) throw error;
      toast({
        title: "Success",
  ...
  if (showEditor) {
    return <DocumentationEditor doc={editingDoc} onSave={handleSaveDoc} onCancel={handleCancelEdit} />;
  }
  return <div className="space-y-6">
      ...
      duration: 0.4
        ...
        <Button onClick={handleCreateDoc} className="bg-brandae-green text-brandae-dark hover:bg-brandae-green/90 rounded">
      ...
      delay: 0.1
        ...
        <div className="relative">
          ...
          <Input placeholder="Search documentation..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-10 bg-brandae-gray border-brandae-green/20 text-white" />
        </div>

        <select value={filterCategory} onChange={e => setFilterCategory(e.target.value)} className="px-3 py-2 bg-brandae-gray border border-brandae-green/20 text-white rounded">
          <option value="">All Categories</option>
          {categories.map(category => <option key={category} value={category}>{category}</option>)}
        </select>

        <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="px-3 py-2 bg-brandae-gray border border-brandae-green/20 text-white rounded">
      ...
      {isLoading ? <div className="text-center py-12">
        ...
        </div> : filteredDocs.length === 0 ? <Card className="bg-brandae-gray border-brandae-green/20">
          <CardContent className="text-center py-12">
            <BookOpen size={48} className="mx-auto text-gray-400 mb-4" />
            <p className="text-gray-400 mb-4">
              {searchTerm || filterCategory || filterStatus ? 'No documentation found matching your filters.' : 'No documentation created yet.'}
            </p>
            {!searchTerm && !filterCategory && !filterStatus && <Button onClick={handleCreateDoc} className="bg-brandae-green text-brandae-dark hover:bg-brandae-green/90">
                <Plus size={20} className="mr-2" />
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      ...
                      <div className="flex flex-wrap gap-2 mb-3">
                        ...
                        {doc.featured && <Badge className="bg-brandae-green text-brandae-dark">
                            Featured
                          </Badge>}
                      ...
                      <div className="flex flex-wrap gap-2">
                        {doc.tags.slice(0, 3).map(tag => <Badge key={tag} variant="outline" className="text-xs border-brandae-green/20 text-gray-400">
                            {tag}
                          </Badge>)}
                        {doc.tags.length > 3 && <Badge variant="outline" className="text-xs border-brandae-green/20 text-gray-400">
