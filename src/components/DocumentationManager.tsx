...
const DocumentationManager = () => {
  ...
  const fetchData = async () => {
    try {
      const [docsResponse, categoriesResponse] = await Promise.all([supabase.from('documentation').select('*').order('created_at', {
        ascending: false
  ...
  const filterDocs = () => {
    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter(doc => doc.title.toLowerCase().includes(query) || doc.excerpt?.toLowerCase().includes(query) || doc.category.toLowerCase().includes(query) || doc.tags.some(tag => tag.toLowerCase().includes(query)));
    }
  ...
  if (showEditor) {
    return <DocumentationEditor doc={editingDoc} categories={categories} onSave={handleSaveDoc} onCancel={() => {
      setShowEditor(false);
      setEditingDoc(null);
    }} />;
  }
  return <div>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="flex-1">
            <div className="relative">
              ...
              <Input placeholder="Search documentation..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-10 bg-brandae-gray border-brandae-green/20 text-white rounded" />
            </div>
          </div>
          <div className="flex items-center gap-2">
            <Filter size={16} className="text-gray-400" />
            <select value={selectedCategory} onChange={e => setSelectedCategory(e.target.value)} className="px-3 py-2 bg-brandae-gray border border-brandae-green/20 text-white rounded">
      ...
      delay: 0.2
          ...
          {filteredDocs.length === 0 ? <Card className="bg-brandae-gray border-brandae-green/20">
              <CardContent className="text-center py-12">
                <h3 className="text-xl font-semibold mb-2 text-white">
                  {searchQuery || selectedCategory !== 'all' ? 'No matching documentation found' : 'No documentation yet'}
                </h3>
                <p className="text-gray-400 mb-4">
                  {searchQuery || selectedCategory !== 'all' ? 'Try adjusting your search or filters' : 'Create your first documentation article to get started'}
                </p>
            ...
            </Card> : filteredDocs.map((doc, index) => <motion.div key={doc.id} initial={{
        opacity: 0,
        y: 20
      }} animate={{
        opacity: 1,
                ...
                <Card className="bg-brandae-gray border-brandae-green/20 hover:border-brandae-green/40 transition-colors">
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        ...
                        <div className="flex flex-wrap gap-1 mb-2">
                          {doc.tags.slice(0, 5).map(tag => <Badge key={tag} variant="secondary" className="text-xs bg-brandae-dark/50 text-gray-300">
                              #{tag}
                            </Badge>)}
                          {doc.tags.length > 5 && <Badge variant="secondary" className="text-xs bg-brandae-dark/50 text-gray-300">
                  ...
                  <CardContent className="rounded">
                    <div className="flex gap-2 flex-wrap">
                      <Button size="sm" variant="outline" onClick={() => handleEdit(doc)} className="border-brandae-green/50 text-brandae-green hover:bg-brandae-green/10 rounded">
                        <Edit size={16} className="mr-1" />
                        Edit
                      </Button>
                      <Button size="sm" variant="outline" onClick={() => togglePublished(doc)} className="border-brandae-green/50 text-brandae-green hover:bg-brandae-green/10 rounded">
                        {doc.published ? <EyeOff size={16} className="mr-1" /> : <Eye size={16} className="mr-1" />}
                        {doc.published ? 'Unpublish' : 'Publish'}
                      </Button>
                      {doc.published && <Button size="sm" variant="outline" asChild className="border-brandae-green/50 text-brandae-green hover:bg-brandae-green/10">
                          <a href={`/docs/${doc.slug}`} target="_blank" rel="noopener noreferrer" className="rounded">
                            <ExternalLink size={16} className="mr-1" />
                            View
                          </a>
                        </Button>}
                      <Button size="sm" variant="outline" onClick={() => handleDelete(doc.id)} className="border-red-500/50 text-red-400 hover:bg-red-500/10 rounded">
