...
export default function DocDetails() {
  ...
  const fetchDoc = async () => {
    try {
      ...
      const {
        data: docData,
        error: docError
      } = await supabase.from('documentation').select('*').eq('slug', slug).eq('published', true).single();
      ...
      if (docData) {
        ...
        await supabase.from('documentation').update({
          views_count: (docData.views_count || 0) + 1
        }).eq('id', docData.id);
        ...
        const {
          data: relatedData,
          error: relatedError
        } = await supabase.from('documentation').select('*').eq('published', true).neq('id', docData.id).eq('category', docData.category).limit(3);
  ...
  return <div className="min-h-screen bg-brandae-dark text-white">
      <SEO title={doc.meta_title || doc.title} description={doc.meta_description || doc.excerpt} keywords={doc.seo_keywords || `${doc.category}, documentation, ${doc.tags.join(', ')}`} />
      ...
      <section className="pb-12 px-6 md:px-12 lg:px-24">
        <div className="container mx-auto">
          ...
          duration: 0.8
            ...
            {doc.featured_image_url && <div className="mb-8 rounded-2xl overflow-hidden">
                <img src={doc.featured_image_url} alt={doc.title} className="w-full h-64 md:h-96 object-cover" />
              </div>}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-12">
            ...
            <div className="lg:col-span-3">
              ...
              delay: 0.2
                ...
                <div className="text-gray-200 leading-relaxed" dangerouslySetInnerHTML={{
                __html: doc.content || ''
              }} />
              </motion.div>
            </div>
            ...
            <div className="lg:col-span-1">
              ...
              delay: 0.4
                ...
                {/* Tags */}
                {doc.tags.length > 0 && <Card className="bg-brandae-gray border-brandae-green/20 mb-8">
                    <CardHeader>
                    ...
                    <CardContent>
                      <div className="flex flex-wrap gap-2">
                        {doc.tags.map(tag => <Badge key={tag} variant="outline" className="border-brandae-green/50 text-brandae-green">
                            {tag}
                          </Badge>)}
                      </div>
                    </CardContent>
      ...
      {relatedDocs.length > 0 && <section className="px-6 md:px-12 lg:px-24 py-20 bg-brandae-darker">
          <div className="container mx-auto">
            <motion.div initial={{
          opacity: 0,
          y: 30
          ...
          duration: 0.8
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {relatedDocs.map((relatedDoc, index) => <motion.div key={relatedDoc.id} initial={{
              opacity: 0,
              y: 20
            }} whileInView={{
              opacity: 1,
              ...
              delay: index * 0.1
                    ...
                    <Card className="bg-brandae-gray border-brandae-green/20 hover:border-brandae-green/40 transition-all duration-300 h-full">
                      <CardContent className="p-0">
                        {relatedDoc.featured_image_url && <img src={relatedDoc.featured_image_url} alt={relatedDoc.title} className="w-full h-48 object-cover rounded-t-lg" />}
                        <div className="p-6">
