  ...
  const fetchSitemapPages = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('sitemap_pages').select('*').eq('is_active', true).order('priority', {
        ascending: false
      });
...
${pages.map(page => `  <url>
  ...
  if (isLoading) {
    return <div className="min-h-screen bg-brandae-dark text-white flex items-center justify-center">
        <div className="text-center">
          <div className="text-xl">Loading sitemap...</div>
        </div>
      </div>;
  }
  return <div className="min-h-screen bg-brandae-dark text-white p-8">
      <div className="container mx-auto max-w-4xl">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-3xl font-bold">Website Sitemap</h1>
          <button onClick={generateXMLSitemap} className="bg-brandae-green text-brandae-dark px-4 py-2 rounded hover:bg-brandae-green/90 transition-colors">
        ...
        <div className="grid gap-6">
          ...
          return <div key={type} className="bg-brandae-gray p-6 border border-brandae-green/20 rounded">
                <h2 className="text-xl font-semibold mb-4 capitalize text-brandae-green">
                  {type.replace('_', ' ')} Pages ({typePages.length})
                </h2>
                <div className="space-y-3">
                  {typePages.map((page, index) => <div key={index} className="flex justify-between items-start p-3 bg-brandae-dark rounded border border-brandae-green/10">
                      <div className="flex-1">
                        <a href={page.url} className="text-brandae-green hover:text-brandae-green/80 font-medium" target="_blank" rel="noopener noreferrer">
                          {page.url}
                        </a>
                        <p className="text-gray-300 text-sm mt-1">{page.title}</p>
                        {page.description && <p className="text-gray-400 text-xs mt-1">{page.description}</p>}
                      </div>
